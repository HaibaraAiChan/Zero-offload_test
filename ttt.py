...
# # GPU_memory = "GPU Memory usage: " + str((info.used) / 1024 / 1024 / 1024) + "GB\n"
# memory = "CPU Memory usage: " + str((total - free) / 1024 / 1024 / 1024) + "GB\nCPU Memory usage percent: %d" % (
# 	int(round(data.percent))) + "%" + "\n"
...
def run(args, device, data):
	...
	for epoch in range(args.num_epochs):
		...
		for step, (input_nodes, seeds, blocks) in enumerate(dataloader):
			...
			#     print('Time per Step (sec) {:.4f}' ,np.mean(avg_step_time))
			#     avg_step_time.clear()
			#     acc = compute_acc(batch_pred, batch_labels)
			#     gpu_mem_alloc = th.cuda.max_memory_allocated() / 1000000 if th.cuda.is_available() else 0
			#     print('Epoch {:05d} | Step {:05d} | Loss {:.4f} | Train Acc {:.4f} | Speed (samples/sec) {:.4f} | Time per Step (sec) {:.4f} | GPU {:.1f} MB'.format(
			#         epoch, step, loss.item(), acc.item(), np.mean(iter_tput[3:]), step_time, gpu_mem_alloc))
			# tic_step = time.time()
			...
			if step % args.log_every==0:
				...
				gpu_mem_alloc = th.cuda.max_memory_allocated() / 1000000 if th.cuda.is_available() else 0
				print(
					'Epoch {:05d} | Step {:05d} | Loss {:.4f} | Train Acc {:.4f} | Speed (samples/sec) {:.4f} | Time per Step (sec) {:.4f} | GPU {:.1f} MB'.format(
						epoch, step, loss.item(), acc.item(), np.mean(iter_tput[3:]), np.mean(avg_step_time),
						gpu_mem_alloc))
				avg_step_time.clear()
			tic_step = time.time()

		print('avg iteration GPU training time:%.8f' % (sum(time2_list[10:]) / len(time2_list[10:])))
	...
	print('avg epoch GPU training time:%.8f' % (sum(time1_list[100:]) / len(time1_list[100:])))
